...
export function renderEllipse(context: CanvasRenderingContext2D, instance: EllipseElement) {
  ...
  context.beginPath()
  context.ellipse((startAxis.x + endAxis.x) / 2, (startAxis.y + endAxis.y) / 2, Math.abs(Math.round((endAxis.x - startAxis.x) / 2)), Math.abs(Math.round((endAxis.y - startAxis.y) / 2)), 0, 0, Math.PI * 2)
  context.closePath()
  context.stroke()
  context.fill()
  context.restore()
